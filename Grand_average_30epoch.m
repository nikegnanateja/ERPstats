...
[files,path] = uigetfile(['H:\Dhatri\sttg cnt\averages set files\avg set only eye & ecg components removed\cut epoch','\*.set'], 'Select the Set files', 'MultiSelect', 'on');
n = length(files);
for i = 1:n
...
ALLEEG(n+1).event = ALLEEG(n+1).event(1:n);
ALLEEG(n+1).epoch = ALLEEG(n+1).epoch(1:n);
