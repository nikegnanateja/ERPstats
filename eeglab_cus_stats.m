...
origpp = clusthresh;
[row,col] = size(clusthresh);
res_pp = reshape(clusthresh,[],1);
[fdr_p,pcor,padj] = fdr(res_pp); %% FDR calculation
fdr_clustr = reshape(padj,row,col);


% plist = clusthresh(:);
% plist(plist==1) = [];
% 
% sort_p = sort(plist,'ascend');
% 
% fdr_list = 0.05*(([1:length(sort_p)]/length(plist)));
% fdr_array = find(sort_p <=fdr_list');
% fdr_ind =length(fdr_array);
% if isempty(sort_p)
%     fdr_clustr= 1;
% else
% fdr_p = sort_p(fdr_ind);
% 
% inds = find(clusthresh>fdr_p);
% % tbackup = tvalue;pbackup = pvalue;
