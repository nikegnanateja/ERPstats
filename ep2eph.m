...
fclose(fid);
   outfilename = strrep(filename,'ep', 'eph');
   
    [numtimeframes, numchannels] = size(thedata);
    
    theheader=[numchannels numtimeframes sf];
    dlmwrite(outfilename,theheader,'\t');
    dlmwrite(outfilename,thedata,'delimiter','\t','-append');
end
